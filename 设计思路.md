...
> **当前阶段**：已实现 `ContextAssembler.assemble()` 与本地冒烟（`python -m context.assembler`）；**尚未**接入 ReActAgent 主循环；associative 图摘要、tool 消息完整字段为 P0 待补。
...
```
context/
├── __init__.py          # 导出 ContextAssembler
...
- [x] `[MEMORY]` / `[DOCUMENTS]` 块与 RAG metadata 格式化
- [x] 历史多角色保留（user/assistant/tool/system 映射）
- [x] `min_relevance` 过滤与 `max_tokens` 预算
...
| 优先级 | 项 |
|--------|-----|
| P0 | 接入 ReActAgent：每轮 `recall` + 可选 `search` → `assemble` → `generate_stream` |
| P0 | history 透传 `tool_calls`、`tool_call_id`、`name`（tool calling 闭环） |
| P0 | `memories` 为 `dict` 时读取 `score` 键（当前仅 `hasattr` 对象属性） |
| P1 | Compress：将 **人设 + [MEMORY] + [DOCUMENTS]** 视为固定头，或分类型裁剪 |
| P1 | Select：**为 history 预留独立 token 配额**，避免被 memory/document 挤光 |
| P1 | `system_reserve_ratio` 真正约束 SYSTEM 类消息总长度 |
| P1 | 注入 **associative** 图摘要（`RecallResult.graph` → SYSTEM 块） |
...
**注意**：Compress 仅锚定第一条 SYSTEM，多块 SYSTEM（记忆/文档）可能被误裁；Select 阶段 history 优先级最低易丢失；dict 记忆分数未参与过滤；图记忆与 Agent 主循环尚未接入。

整体而言，**context 层已具备可集成的上下文装配骨架**，是灵枢「memory + retrieval → LLM」链路上的 **最后一环拼 prompt**；下一步是在中枢内固定 **何时 recall / search、如何传入 assemble**，并补齐 tool 消息与图记忆字段。
