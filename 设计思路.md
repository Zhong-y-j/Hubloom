...
整体而言，**memory 层已具备四种记忆的生产级骨架**，并与 ReAct 意图澄清阶段完成 **读（预取）+ 写（提炼 + link）** 闭环；下一步重点是 Hub 侧路由与 PlanExecute 消费结构化意图。
...
**注意**：Select 阶段 history 优先级仍最低，预算紧张时远历史易丢失；**Hub 路由器尚未实现**（见第六篇 PlanExecute 已可独立冒烟）。

整体而言，**context 层已与 ReAct 意图澄清闭环**，是灵枢「memory + retrieval → LLM」链路上的 **拼 prompt 最后一环**；PlanExecute 当前以 `StructuredIntent` + 子 Agent 上下文为主，尚未在每步 Execute 中复用 ContextAssembler。
...
> **当前阶段**：已实现 **ReAct 意图澄清最小闭环**（第五篇）、**PlanExecute 规划执行 MVP**（第六篇）、**Reflection LLM 审查 MVP**（第七篇）；**尚未**实现 **Hub 路由器**（串联三阶段）及 Reflection 打回后的 PlanExecute 重跑。
...
用户输入
    │
    ▼
Hub（规划）                    ← 尚未实现：读 intent 决定下一步
    │
    ├──► ReActAgent（第五篇）    意图澄清专家
    │         └── StructuredIntent
    │
    └──► PlanExecuteAgent（第六篇）  规划 → 分发 → 汇总
              └── ExecutionResult → ReflectionAgent（第七篇）→ Hub / 用户
...
├── events.py            # AgentEvent 族（含 Plan/Step 流式事件）
├── test.py              # ReAct 冒烟
...
- [x] **Reflection MVP**（详见 **第七篇**）：`ReflectionAgent` + `ReflectionVerdict` + `agents/test_reflection.py`

### 9.2 待办（接 Hub / 与第六、七篇衔接）

| 优先级 | 项 |
|--------|-----|
| P0 | **Hub 路由器**：`is_clear` + `intent` → ReAct / PlanExecute / Reflection；`verdict.passed` → 交付或重跑 Plan |
| P1 | 路由文案：`is_clear=false` 显示「继续澄清」而非「直接回复」 |
| P1 | `consolidate_only_when_clear`：澄清阶段跳过长期记忆写入 |
| P1 | link 策略收紧：正文须含实体名才 `link_memory` |
| P2 | 多轮 CLI REPL（同 `session_id` 连续 `run_stream`） |
| P2 | Hub 内 Reflection 打回 → PlanExecute 指定 step 重跑（优化闭环） |
...
**注意**：Hub 未实现时 `agents/test.py` 仅单轮 ReAct 演示；`agents/test_plan_execute.py` 绕过 Hub 直接喂 `StructuredIntent`；澄清阶段写长期记忆可能过早；Reflection 仍为规划。

整体而言，**ReAct + PlanExecute + Reflection 均已具备可单独冒烟与集成的骨架**；灵枢下一里程碑是 **Hub 串联**（见第七篇 §八）。
...
> **当前阶段**：已实现 **PlanExecute MVP**（LLM 规划 + Registry 解析 + `RegistryStepDelegate` 流式执行 + 默认汇总）；**Reflection 审查**见第七篇；**尚未**实现 Hub 自动路由、Execute 步级 ContextAssembler / RAG 预取、无依赖步骤并行执行、Reflection 打回重跑。
...
- 测试脚本 `agents/test_plan_execute.py` 直接构造 intent，**等价于手动扮演 Hub**。
...
```python
# Hub 侧目标形态（尚未实现 hub.py）
intent = react_agent.get_last_intent()
if intent and intent.is_clear and intent.should_invoke_plan():
    async for ev in plan_execute_agent.execute_stream(intent):
        ...  # 消费 PlanCreated / StepOutputDelta / ExecutionResultEvent
```
...
流程：打印 `StructuredIntent` → Registry → **Plan 流式** → Plan 解析表 → **Execute 逐步流式** → 统计与 deliverable 摘要。
...
| 优先级 | 项 |
|--------|-----|
| P0 | **Hub 路由器**：自动串联 ReAct → PlanExecute → Reflection |
...
| P1 | Hub：**verdict.passed=false** 时按 `related_step_ids` 重跑 PlanExecute |
| P2 | 持久化 Registry / 专业 Agent 配置外置（YAML 或 DB） |
| P2 | PlanExecute 回合结束写 episodic（任务级记忆） |
...
**优势**：与 ReAct 职责分离清晰；`StructuredIntent` → `ExecutionPlan` → 专业 Agent 链路可观测（流式事件齐全）；Registry + Delegate 双扩展点；依赖步骤全文下传，多步合同/文档场景可跑通。

**注意**：Hub 未实现时需手动构造 intent；Aggregator 默认拼接所有成功步，最终交付物可能冗长；`partial_success` 命名易误解；Plan 与 Execute 未共享 ReAct 的 GSSC 预取，长文档任务需后续补强。

整体而言，**PlanExecute 已具备与 ReAct、Reflection 对接的骨架**；灵枢下一里程碑是 **Hub 串联** 与按任务类型优化 **deliverable 汇总策略**。
...
> **当前阶段**：已实现 **Reflection MVP**（L0 规则前置 + LLM 流式审查 + `` ```reflection` `` JSON 解析 + `agents/test_reflection.py` 独立冒烟）；**尚未**实现 Hub 自动串联、审查不通过后的 PlanExecute 指定步骤重跑、对照 RAG 的复核 Tool。
...
## 六、Hub 消费方式（目标形态，尚未实现 `hub.py`）

```python
result = plan_execute.get_last_result()
async for ev in reflection.review_stream(result):
    ...  # UI：ReflectionTextDelta / Complete
verdict = reflection.get_last_verdict()

if verdict.passed:
    # 向用户展示 result.deliverable
    ...
else:
    # 优化：Hub 调 PlanExecute，将 verdict.recommendation / issues 写入重跑上下文
    # 例如重跑 related_step_ids 中的步骤（待 PlanExecute 支持 revision_feedback）
    ...
```
...
| 优先级 | 项 |
|--------|-----|
| P0 | **Hub**：PlanExecute 结束后自动 `review_stream`；按 `verdict.passed` 分支 |
| P1 | PlanExecute：**指定 step 重跑** + 注入 `revision_feedback`（来自 `issues` / `recommendation`） |
| P2 | Reflection 可选对照 `search_documents`（只读） |
...
**优势**：输入输出边界清晰（`ExecutionResult` → `ReflectionVerdict`）；只读审查不改稿，与 PlanExecute 职责分离；`related_step_ids` 为 Hub 优化预留；L0+L1 分层节省成本；事件流与 Plan/ReAct 风格一致。

**注意**：独立测试需手写或从 Plan 复制 `ExecutionResult`；`fixture=pass` 不保证 `passed=true`（审查可能发现真实一致性问题）；`review_report` 含重复 JSON 时以解析后的 `verdict` 为准；优化闭环依赖 Hub + PlanExecute 扩展，Reflection 本身已闭环。

整体而言，**Reflection 已具备可接入 Hub 的审查骨架**；灵枢下一里程碑是 **Hub 串联三阶段** 与 **不通过时的 PlanExecute 重跑**。
