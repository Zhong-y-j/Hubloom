...
**注意**：Select 阶段 history 优先级仍最低，预算紧张时远历史易丢失；Hub / PlanExecute 尚未实现。

整体而言，**context 层已与 ReAct 意图澄清闭环**，是灵枢「memory + retrieval → LLM」链路上的 **拼 prompt 最后一环**；下一步是 PlanExecute 复用同一装配模式，并补齐 tool 消息字段。
...
> **当前阶段**：已实现 **ReAct 意图澄清最小闭环**（预取上下文 → 流式推理 → 只读 Tool → 结构化 intent → 记忆提炼）；**尚未**实现 Hub 路由与 **PlanExecute** 消费 `StructuredIntent`。
...
Hub（规划）                    ← 尚未实现：读 intent 决定下一步
    │
    ▼
ReActAgent（本文档）           意图澄清专家
    │
    ├── context/               MemoryContextProvider + ContextAssembler → messages
    ├── memory/                预取 + 回合结束 MemoryConsolidator 写回
    ├── retrieval/             预取 documents + SearchDocumentsTool
    ├── core/LLM               generate_stream + tool calling
    └── agents/intent.py       StructuredIntent 解析与输出规范
    │
    ▼
StructuredIntent               handoff → PlanExecute / 直接回复（general_chat）
```
...
├── events.py            # AgentEvent 族（含 IntentOutcomeEvent、MemoryConsolidatedEvent）
├── test.py              # 冒烟：AgentRunPrinter + 端到端 trace
└── __init__.py
...
### 9.2 待办（接 Hub / PlanExecute）

| 优先级 | 项 |
|--------|-----|
| P0 | **Hub 路由器**：`is_clear` + `intent` → 继续 ReAct / 直接回复 / PlanExecute |
| P0 | **PlanExecuteAgent**：消费 `StructuredIntent.slots` + `summary` |
...
**注意**：Hub 未实现时 test 仅单轮演示；澄清阶段写长期记忆可能过早；`general_task` 在 `is_clear=false` 时路由文案易误解；PlanExecute / Reflection 仍为规划。

整体而言，**意图澄清智能体已具备可集成骨架**；灵枢下一里程碑是 **Hub + PlanExecute** 闭环。
