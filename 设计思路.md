...
> **当前阶段**：已实现 **ReAct**（第五篇）、**PlanExecute**（第六篇）、**Reflection**（第七篇）、**Hub 编排**（第八篇，含审查打回后的指定步骤修订重跑）。端到端入口：项目根目录 **`main.py`**（`agents/` 目录结构见 `agents/README.md`）。
...
Hub 侧：agents/hub/ + agents/app/bootstrap.py + main.py + agents/scripts/
```
...
- [x] **Hub 编排**（详见 **第八篇**）：`CortexHub` + `main.py`（`--repl`）
...
## 六、Hub 消费方式（已实现 · `agents/hub/hub.py`）
...
> **当前阶段**：已实现 `agents/hub/`、`agents/app/bootstrap.py`、根目录 **`main.py`**（单轮默认 + `--repl` 多轮）。入口：`PYTHONPATH=. python main.py` 与 `PYTHONPATH=. python main.py --repl`。
