...
Hubloom 不仅是「用自然语言操作 API」，更是企业智能体技术栈中的**编排与互联枢纽**。**MCP** 负责连接工具与数据（已支持）；**A2A** 为下一步推进方向，用于跨 Agent 任务委托；**ANP** 在路线图中，面向更开放的 Agent 互联协作。
...
Hubloom 采用分层设计：用户侧 Web 对话 → **ADP 编排**（快答 / 深思考）→ **MCP 适配**（Swagger 转工具、代理企业 API）→ 可选的**长期记忆**与 **RAG** 增强；**A2A** 为跨 Agent 互联方向（设计中）。各层职责与链路详见 [docs/](./docs/)：

| 文档                                      | 说明                                       |
| ----------------------------------------- | ------------------------------------------ |
| [总体架构图](./docs/Hubloom总体架构图.md) | 系统分层、内部展开、深度思考时序           |
| [ADP 编排](./docs/Hubloom-ADP编排.md)     | Assessor 路由、Chat / Thought 双路径       |
| [MCP 适配](./docs/Hubloom-MCP适配.md)     | OpenAPI 管线、Gateway / Worker、Token 透传 |
| [A2A 互联](./docs/Hubloom-A2A互联.md)     | 双向 A2A 设计：入站 Server / 出站 Client   |
