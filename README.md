...
**业务场景**：用户的初始输入往往是模糊、不完整的。例如，“帮我写个合同”、“我想学做川菜”。中枢不能直接跳转到任务分发，而必须先与用户交互，澄清真实需求。

**核心职责**：

- 识别用户意图是否明确。
- 对于模糊需求，通过多轮对话追问关键信息（如合同类型、特殊条款、预算等）。
- 提取并结构化用户需求，形成“明确的任务描述”。

**工作模式**：
ReAct（思考-行动-观察）非常适合这种交互式场景。它会主动向用户提问，根据回答动态调整对需求的理解，直到获得足够清晰的信息，才将任务传递给下一阶段。

**输入**：用户原始输入（可能包含歧义、缺失信息）。
**输出**：结构化的意图描述，例如 `{“意图”: “合同撰写”, “类型”: “软件开发合同”, “必须条款”: [“源代码归属”], “预算”: 5000}`。
...
| 阶段                 | 核心内容                                                                    | 关键交付                          |
| -------------------- | --------------------------------------------------------------------------- | --------------------------------- |
| 1. 中枢骨架搭建      | 实现Agent Registry、意图澄清（ReAct）、任务规划（PlanExecute）的骨架逻辑。  | 可运行的灵枢中枢原型              |
...
| 5. 记忆与上下文集成  | 将ConversationStore和MemoryManager集成到中枢，实现多轮对话和长期记忆。      | 中枢具备持续学习和个性化能力      |
