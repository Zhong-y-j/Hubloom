...
**灵枢（Agent Cortex）是一个 OpenAPI 驱动的对话式 API 网关**

> 给企业现有 REST 后台接一层自然语言入口，用户用说话的方式查数据、调接口，而无需改动原有业务系统。传入 Swagger/OpenAPI，自动生成可调用的工具集；通过评估路由 → 快答 / 深度思考两条路径，在对话中安全、可解释地调用企业 API，并支持会话、记忆与知识库增强。
