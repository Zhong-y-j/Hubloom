(function () {
  const AUTH_TOKEN = "transport_demo_token";
  const AUTH_USER = "transport_demo_user";
  const MCP_SWAGGER = "transport_mcp_swagger_url";
  const MCP_BASE = "transport_mcp_base_url";
  const MCP_SCHEME = "transport_mcp_auth_scheme";

  const CONFIG_KEYS = {
    openaiApiKey: "transport_openai_api_key",
    openaiModel: "transport_openai_model",
    openaiBaseUrl: "transport_openai_base_url",
  };
  ...
  const CHAT_PLACEHOLDER_DISCONNECTED = "正在准备交通助手，请稍候…";
  ...
  function getEmptyStateHtml() {
    if (swaggerConnected) {
      return (
        ...
        '<p class="empty-desc">用自然语言查询高铁行程、车次状态与订单。</p>' +
        '<p class="empty-hint">例如：「高铁 G1234 今天延误情况？」' +
        "「查 HTL-889 订单详情」</p>"
      );
    }
    return (
      '<p class="empty-title">正在准备助手</p>' +
      '<p class="empty-desc">请确认 Hubloom 与交通服务均已启动，并填写 OpenAI API Key。</p>'
    );
  }
