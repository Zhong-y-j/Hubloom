...
读完 Skill 正文后，仍靠 **`list_tools` / `call_tool`**（MCP）办实事，或交 Respond 说明 / 出 A2UI。
...
2. 按正文办事：该拒就拒、该 `list_tools`/`call_tool` 再调、该交 Respond 就停
...
- Hubloom 主路径是 OpenAPI → MCP → `call_tool`；领域规程用 Markdown Skill + `read_skill` 已够用
