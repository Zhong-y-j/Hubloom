...
from tools.param_hints import format_tool_param_hints
...
def _format_tool_line(d: dict) -> str:
    ...
    hints = format_tool_param_hints(params if isinstance(params, dict) else {})
    line = f"- **{d['name']}**：{d.get('description', '').strip()}"
    if hints:
        line += f"\n  需用户提供：{hints}"
    return line
...
def _default_allowed_tools(tools: ToolRegistry) -> set[str]:
    return {d["name"] for d in tools.list_definitions()}
