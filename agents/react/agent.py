...
## 工具（MCP 动态目录）
- 「当前可调用工具」「系统执行能力」均来自 **MCP 注册表**（随接入的 Swagger/OpenAPI 变化），以本消息下列表为唯一依据。
- 本阶段仅可**直接调用**「当前可调用工具」中的只读检索类工具。
- 「系统执行能力」用于理解系统能做什么、写入 intent.slots.suggested_tools（name 须与列表一致）；澄清阶段**禁止**直接调用。
- **禁止**假设 Petstore、某一固定业务 Swagger 或训练数据里的工具名；列表中没有的能力不要承诺。
    ...
    if callable_defs or cap_defs:
        parts.extend(
            [
                ...
                "- **确认需求后可做**：仅根据「系统执行能力」各条 description 归纳 2～5 条用户可发起的任务（勿举例目录中不存在的场景）。",
                "- 只介绍下列列表中有的能力；工具 name 不要原样暴露给用户，用自然中文描述即可。",
            ]
        )
    ...
    def __init__(
        ...
        *,
        ...
        if system_prompt is not None:
            ...
            base = _build_default_system(tools, allowed_tools)
    ...
    async def run_stream(self, task: str) -> AsyncIterator[AgentEvent]:
        ...
        for _ in range(self.max_steps):
            ...
            async for ev in self.llm.generate_stream(
                messages=self.get_history(),
                tools=_callable_tool_defs(self.tools, self.allowed_tools) or None,
            ):
