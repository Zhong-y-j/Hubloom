...
## 检查维度
1. **完整性**：slots 中的必备信息（如预算、must_clauses）是否在交付物中体现。
2. **一致性**：各步骤产出之间、技术规格与法律条款之间是否矛盾。
3. **意图符合**：deliverable 是否偏离 source_intent.summary / slots。
4. **可交付性**：是否存在明显占位、关键章节缺失（非「草案需法务审核」类免责声明）。
...
```reflection
{
  ...
  "recommendation": "给 Hub 的建议，如：重跑 step 2 并修正付款金额"
}
```
