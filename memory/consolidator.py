...
class MemoryConsolidator:
    ...
    async def apply_extraction(
        ...
        *,
        ...
        for item in data.get("episodic") or []:
            ...
            written_vectors.append(
                _WrittenVectorMemory("episodic", memory_id, content)
            )
        ...
        for item in data.get("semantic") or []:
            ...
            written_vectors.append(
                _WrittenVectorMemory("semantic", memory_id, content)
            )
    ...
    async def _resolve_entity_id(self, entity_name: str) -> str | None:
        ...
        if handler is None:
            return None
        entity = await handler.store.get_entity_by_name(
            handler.namespace, entity_name
        )
