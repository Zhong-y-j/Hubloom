...
class MemoryManager:
    ...
    async def remember(
        ...
        *,
        ...
        Args:
            ...
            source: 记忆来源
            metadata: 记忆元数据
            token_count: 记忆 token 数量
            turn_index: 记忆 turn 索引
        """
        ...
        return await handler.remember(content=content, source=source, metadata=metadata)
    ...
    async def recall(
        ...
        *,
        ...
        per_handler_k = top_k if mode != "hybrid" else max(top_k, min(top_k * 2, 20))
    ...
    async def forget(
        ...
        if memory_type == "conversation":
            raise ValueError(
                "conversation 不支持 forget，请用 clear_all(memory_type='conversation')"
            )
        handler = self.handlers.get(memory_type)
