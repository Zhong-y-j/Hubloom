...
def _emit(prefix: str, enabled: bool, message: str, /, **fields: object) -> None:
    if not enabled:
        return
    tid = _turn_id.get()
    if tid:
...
def hub_log(message: str, /, **fields: object) -> None:
    _emit("hub", _channel_on(_hub_log), message, **fields)


def plan_log(message: str, /, **fields: object) -> None:
    _emit("plan", _channel_on(_plan_log), message, **fields)


def reflection_log(message: str, /, **fields: object) -> None:
    _emit("reflection", _channel_on(_reflection_log), message, **fields)


def specialist_log(message: str, /, **fields: object) -> None:
    _emit("specialist", _channel_on(_specialist_log), message, **fields)


def react_log(message: str, /, **fields: object) -> None:
    _emit("react", _channel_on(_react_log), message, **fields)


def mcp_log(message: str, /, **fields: object) -> None:
    _emit("mcp", _channel_on(_mcp_log), message, **fields)


def memory_log(message: str, /, **fields: object) -> None:
    _emit("memory", _channel_on(_memory_log), message, **fields)


def cortex_log(message: str, /, **fields: object) -> None:
    _emit("cortex", _channel_on(_cortex_log), message, **fields)


def agent_trace(message: str, /, **fields: object) -> None:
    _emit("agent", _agent_log, message, **fields)


def a2a_log(message: str, /, **fields: object) -> None:
