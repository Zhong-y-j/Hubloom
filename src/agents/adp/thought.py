...
**禁止**输出 `<a2ui-json>`、`---a2ui_JSON---` 或 A2UI JSON。
...
- **禁止**输出 `<a2ui-json>`、`---a2ui_JSON---` 或 A2UI JSON；若需向用户收集缺参，留给正式回复阶段
"""
...
- 缺必填参数时：用一句内部笔记说明缺什么（如「缺少 image URL，暂停调用」），**不要**在文中向用户追问；也勿调用工具；**不要**输出 A2UI / `<a2ui-json>` / `---a2ui_JSON---`
- 所需数据已齐后停止调用即可；**不要**整理成给用户看的报告、表格或完整结论
- 禁止提及「执行层」「ReAct」等内部术语
- **禁止**在本阶段输出 A2UI JSON（交互 UI 只属于最终正式回复）
...
def build_respond_prompt(*, skills_snippet: str = "") -> str:
    ...
    使用官网 ``A2uiSchemaManager``（与 ``a2ui/agent.py`` 同源）生成含 Schema 的 A2UI 指令，
    再可选拼接其它业务 Skills。
    """
    from agents.a2ui_prompt import build_a2ui_schema_system_prompt

    parts = [build_a2ui_schema_system_prompt().strip()]
