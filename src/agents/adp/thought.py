...
_DELIBERATE_BEFORE = """你是 Hubloom，正在心里盘算接下来如何处理任务，并把思路写成**内部工作笔记**。
...
结合「可用工具」与「API 分组」判断：
- 有匹配工具：说明我打算如何调用（企业 API：先根据 tag 选定分组，再 list_tools → **必须** call_tool）
...
**禁止**输出 `---a2ui_JSON---`、A2UI 消息数组或任何给用户看的表单 JSON；缺参时只写「缺哪些字段，留待正式回复用表单收集」。
"""

_DELIBERATE_REPLAN = """你是 Hubloom。先前执行遇到问题，请用**内部工作笔记**简要记录你将如何调整后续步骤。

要求：一两句第一人称中文；只讲调整方向；**禁止**第二人称（您/你）及面向用户的口吻；可引用可用工具名。
**禁止**输出 `---a2ui_JSON---` 或 A2UI JSON。

若先前失败原因为未登录/身份认证不足：**不要**规划登录、验证码或 token 换取；写明「鉴权不足，应结束执行并由正式回复告知用户」即可。
"""

_DELIBERATE_AFTER = """你是 Hubloom。工具执行已结束，请用**内部工作笔记**简要记录刚才做了什么、结果如何。
...
- **禁止**输出 `---a2ui_JSON---` 或 A2UI JSON；若需向用户收集缺参，留给正式回复阶段
"""

_EXECUTE = """你是 Hubloom 的执行阶段。根据**本轮**任务调用可用工具获取真实数据。
...
- 缺必填参数时：用一句内部笔记说明缺什么（如「缺少 image URL，暂停调用」），**不要**在文中向用户追问；也勿调用工具；**不要**输出 A2UI / `---a2ui_JSON---`
- 所需数据已齐后停止调用即可；**不要**整理成给用户看的报告、表格或完整结论
- 禁止提及「执行层」「ReAct」等内部术语
- **禁止**在本阶段输出 `---a2ui_JSON---` 或任何 A2UI JSON（表单只属于最终正式回复）

企业 API（下方有「API 分组」时）：
- 根据分组目录选定 tag，先 list_tools(tag) 查看工具名与参数 schema
- **随后必须** call_tool(tag, tool_name, arguments) 完成实际业务调用；无参接口省略 arguments
- list_tools **仅**用于发现工具与参数，**不算**完成任务；禁止只 list_tools 后用「正在创建/已创建/正在查询」等文字结束
- 创建、更新、删除、查询等一切读写操作，都必须以 call_tool 的返回为准；未拿到 call_tool 结果前不得宣称操作已完成
- 同一 tag 通常 list_tools 一次即可，找到匹配工具后**立即** call_tool，不要反复 list 而不调用
- arguments 必须是 JSON 对象，勿传字符串形式的 "{}"
...
def is_unauthenticated_tool_result(result: str) -> bool:
    ...
    if isinstance(body, dict):
        ...
        if (
            ...
            and any(
                k in msg
                for k in ("身份认证", "未登录", "未提供", "token", "Token", "认证")
            )
        ):
            return True
...
_META_GATEWAY_TOOLS = frozenset({"list_tools"})
_OBS_TOOL_RE = re.compile(r"^- (.+?)（")


def observation_tool_name(observation: str) -> str:
    """从观察条目中解析工具名（``- {name}（成功）：…``）。"""
    match = _OBS_TOOL_RE.match((observation or "").strip())
    return match.group(1).strip() if match else ""


def has_business_tool_in_observations(
    observations: list[str],
    *,
    since_index: int = 0,
) -> bool:
    """观察中是否已有非元工具（call_tool 业务接口）的执行记录。"""
    for obs in observations[since_index:]:
        name = observation_tool_name(obs)
        if not name or name in _META_GATEWAY_TOOLS or name.startswith("系统"):
            continue
        return True
    return False


def has_list_tools_only_since(
    observations: list[str],
    *,
    since_index: int,
) -> bool:
    """本轮是否只调用了 list_tools，尚未 call_tool 业务接口。"""
    new_obs = observations[since_index:]
    if not new_obs:
        return False
    saw_list_tools = False
    for obs in new_obs:
        name = observation_tool_name(obs)
        if name in _META_GATEWAY_TOOLS:
            saw_list_tools = True
            continue
        if name and not name.startswith("系统"):
            return False
    return saw_list_tools
...
_EXECUTE_CALL_TOOL_NUDGE = """【系统提醒】你已调用 list_tools 查看 schema，但尚未 call_tool 执行实际业务接口。
list_tools 不能代替创建、查询、更新或删除。请根据 schema 立即 call_tool(tag, tool_name, arguments) 完成用户任务。
禁止用「正在创建/正在更新」等进度话术代替 call_tool。仅当缺必填参数、无法构造 arguments 时，用一句内部笔记说明缺什么后结束。"""

_MAX_EXECUTE_TOOL_NUDGES = 2

_RESPOND = """你是 Hubloom，面向用户的智能助手。请根据用户任务与已执行的工具结果，给出最终正式回复。

要求：
- 语气自然、专业，直接回答用户问题
- 仅依据工具返回的真实数据作答，不要编造
- 若无任何工具执行结果，如实说明暂时无法确认相关数据，不要引用历史对话中的业务数字充数
- 创建/更新/删除类任务：执行观察中须出现对应业务工具（call_tool 调用的 tool_name，**非** list_tools）的成功结果，才能告知用户操作已完成
- 若观察里只有 list_tools、没有业务 call_tool 结果，应说明尚未实际执行写操作，**禁止**声称「已创建」「正在创建中」「已更新」等
- 数据不足时：优先按 Skills 中的 A2UI 约定，在本段最终回复输出「简短 Markdown + `---a2ui_JSON---` + 表单」；不要只写长列表追问
- A2UI（若需要）**必须写在本段最终回复**，不要假定思考过程里已经出过表单
- 不要提及内部流程、研判、执行层等术语
...
def build_deliberate_prompt(
    ...
    *,
    ...
    拼接顺序：阶段基座 → API 分组目录 → 可用工具简表。
    注意：不注入完整 Agent Skills（尤其 A2UI few-shot），否则模型会在思考区照抄表单 JSON。
    """
    base = {
...
def build_execute_prompt(
    ...
    *,
    ...
    拼接顺序：执行基座 → API 分组目录 → 可用工具简表。
    不注入完整 Agent Skills（A2UI 仅属于最终正式回复）。
    """
...
def build_respond_prompt(*, skills_snippet: str = "") -> str:
    """组装最终用户回复阶段 system prompt。

    拼接顺序：回复基座 → Skills（A2UI 等输出约定仅在此注入）。
    """
    parts = [_RESPOND.strip()]
...
def _execute_user_message(task: str, observations: list[str]) -> str:
    text = (task or "").strip()
    header = (
        "【本轮任务 — 必须通过 call_tool 获取真实业务结果，勿引用历史 assistant 中的业务结论】\n"
        "list_tools 仅查 schema，不算完成任务；创建/查询/更新/删除均须 call_tool。\n"
        f"{text}"
    )
    if not observations:
        return header
...
def _respond_messages(
    ...
    *,
    ...
    system = build_respond_prompt(skills_snippet=skills_snippet)
    if not observations:
        return [
...
class Thought:
    ...
    async def run_stream(self, messages: list[Message]) -> AsyncIterator[AgentEvent]:
        ...
        tool_count = len(self.tools.list_definitions()) if self.tools is not None else 0
        cortex_log(
    ...
    async def execute(
            ...
            self._execute_messages.append(
                Message(
                    ...
                    content=(
                        ...
                        "优先补齐未完成的 call_tool 业务调用（list_tools 不算已完成）。"
                    ),
                )
            )
        else:
        ...
        for step in range(1, self.max_execute_steps + 1):
            ...
            if final_stop == StopReason.STOP:
                ...
                business_called = has_business_tool_in_observations(
                    self._observations,
                    since_index=observations_at_start,
                )
                list_tools_only = has_list_tools_only_since(
                    self._observations,
                    since_index=observations_at_start,
                )
                tools_called_this_pass = len(self._observations) > observations_at_start
                needs_nudge = (
                    not business_called
                ...
                if needs_nudge:
                    ...
                    nudge = (
                        _EXECUTE_CALL_TOOL_NUDGE
                        if list_tools_only
                        else _EXECUTE_TOOL_NUDGE
                    )
                    self._execute_messages.append(
