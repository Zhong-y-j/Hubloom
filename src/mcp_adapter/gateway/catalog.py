...
_CATALOG_USAGE = (
    "调用方式：根据上表选定 tag，调用 list_api(tag) 查看工具名与参数，"
    "再用 call_api(tag, tool_name, arguments) 调用实际业务接口；无参接口可不传 arguments。"
    "list_api 仅用于查 schema，创建/查询/更新/删除都必须通过 call_api 完成。"
)
