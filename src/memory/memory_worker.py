...
from agent.agent_log import clip, memory_log
...
class WorkerConfig:
    ...
    def from_hubloom_config(cls, cfg: "HubloomConfig") -> WorkerConfig:
        ...
        min_turns = (
            cfg.consolidate_min_turns if cfg.consolidate_min_turns is not None else 3
        )
        return cls(
...
class MemoryMaintenanceWorker:
    ...
    async def _list_sessions(self, session_id: str | None) -> list[dict[str, object]]:
        if session_id:
            count = await asyncio.to_thread(self._conversation_store.count, session_id)
            if count <= 0:
                return []
