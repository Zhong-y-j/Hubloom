...
class Neo4jStore:
    ...
    Actions:
        ensure_schema: 确保 schema 已创建
        upsert_entity: 合并实体节点
        get_entity_by_name: 根据名称获取实体
        relate: 确保两端实体存在并合并关系边
        link_memory: 为实体挂载指向 episodic/semantic 的 ``MemoryRef`` 节点
        recall_neighbors: 按实体名/别名找种子节点，并扩展 ``RELATES_TO`` 邻域（1–2 跳）
        delete_entity: 删除实体
        clear_namespace: 删除该 namespace 下全部 Entity 与 MemoryRef 节点
        close: 关闭数据库连接
    """
