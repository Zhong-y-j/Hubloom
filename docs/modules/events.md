...
读完上面，你应能用自己的话回答：Events 解决什么问题、和聊天入口差在哪、为什么要有 `event_id` 与 `session_id`、规程写在哪里、结果去了哪里。下面开始是代码地图：文件在哪、调用怎么走、本地怎么验。

---

## 本章怎么读

本章后半是 **`src/events/` 的代码地图**：Webhook 事件规范化 → Redis 幂等与会话串行 → 注入分册 → 跑一轮 Agent；可选回调业务方。

HTTP 路由在示例站（`POST /v1/events`），本模块不依赖 FastAPI。调度层通过 `EventAgentRunner` 注入「跑一轮」能力，**不** import `HubloomRuntime`，避免环依赖；示例站用 `StreamHostAgentRunner` 包装 Runtime。
