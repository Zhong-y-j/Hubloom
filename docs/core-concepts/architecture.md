...
不讲协议细节和源码目录——那些分别见 [Runtime](runtime.md)、[Agent](agent.md)、[MCP](mcp-protocol.md)、[Skill](skill.md) 与 [模块导读](../modules/README.md)。
...
- **Runtime** — 装配能力，按会话把一轮交给 Agent
- **Agent** — 决定调工具、追问、确认或收工；与 MCP 多轮来回
- **MCP** — 按 OpenAPI/Swagger 调用企业 HTTP，把结果交回 Agent（图上不单独画出企业系统）
- **回传** — Agent 收工或需要追问时，用 Markdown + SSE 经 BFF 回到前端
...
**Runtime**  
可嵌入的办事内核：装配配置、Agent、工具、会话等。Serve 是一种宿主；你也可以在自有应用里直接持有 Runtime。见 [Runtime](runtime.md)。

**Agent**  
单轮/多轮决策中枢。根据用户话、历史、工具结果与 Skill，选择下一步动作：调用工具、向用户追问、等待确认，或结束并回复。见 [Agent](agent.md)。
...
配置与联调细节见 [进阶功能](../advanced/README.md)。入门阶段只需知道：它们最终都进同一套 Runtime / Agent。
