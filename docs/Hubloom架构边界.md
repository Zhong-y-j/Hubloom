...
| **Swagger → API** | 企业 API 调用主路径 | `src/mcp_adapter/` + `tools/builtin/meta_tools.py` |
...
**现状要点**：`mcp_adapter` 已是主路径；Agent 只见 `list_tools` / `call_tool`；`gateway/` 名存实亡（ predominantly catalog）。

**建议**

1. 文档与目录命名对齐「catalog + full worker」，不再宣传多 Worker Gateway。
2. 主路径只保留 meta-tools；`tools/builtin/mcp_tool.py` 若无引用则降级为内部/测试或删除。
...
2. **MCP 清理**：catalog 命名、meta-tools 单轨、runtime 单入口。
